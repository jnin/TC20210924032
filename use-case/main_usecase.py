...
lmda = [5] # parámetro que multiplica la regularización 
...
n_samples_iter = [6000] # only one value allowed!
...
###---- VALIDATION
n_runs = 10 #number independent runs
...
if __name__ == "__main__":
    ...
    if test_mode:
            results_folder_name = 'results/use_case'
    else:
    ...
    for dataset, original_model in zip(datasets, original_models):
        ...
        for month in months:
            data_filenames='../sampling/use_case/'+year+'_'+month
        ...
        idx = np.random.choice(np.arange(len(xx)), n_samples_iter[0], replace=False)
        xx = xx[idx]
        yy = yy[idx]
            ...
            for n in n_samples_iter:
                for thresh in thresh_vec:
                    ...
                    with Parallel(n_jobs=n_jobs) as parallel:
                        parallel(delayed(sc.sequential_train_use_case)(d, 
                                                               n_classes, 
                                                               learning_rate, 
                                                                 n, 
                                                                 plot=False, 
